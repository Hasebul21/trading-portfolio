...
const LINKS = [
  ...
  { href: "/long-term", label: "Long Term" },
  {
    ...
    linkClassName:
      "px-2 py-1.5 text-[10px] font-semibold leading-snug sm:px-2.5 sm:py-2 sm:text-[11px] sm:leading-tight",
  },
  { href: "/knowledge", label: "Knowledge" },
] as const;

const stripClass =
  "flex w-full max-w-5xl flex-wrap items-center justify-center gap-0.5 rounded-xl border border-zinc-200/90 bg-zinc-50/90 p-0.5 shadow-inner shadow-zinc-900/[0.03] dark:border-zinc-700/80 dark:bg-zinc-900/55 dark:shadow-black/20";
    ...
    <nav aria-label="Main" className={stripClass}>
      {LINKS.map((item) => {
        ...
        const base = linkClassName
          ? linkClassName
          : "px-2.5 py-1.5 text-[12px] font-semibold tracking-tight sm:px-3 sm:py-2 sm:text-[13px]";
        const state = active
