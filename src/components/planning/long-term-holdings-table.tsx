...
export function LongTermHoldingsTable({ rows }: { rows: LongTermHoldingRow[] }) {
 ...
 <span
 className={`tabular-nums ${totals.buys > 0 ? "text-[var(--gain-700)]" : ""}`}
 >
 {totals.buys}
 </span>
 </KpiCell>
 ...
 <span
 className={`tabular-nums ${totals.sells > 0 ? "text-[var(--loss-700)]" : ""}`}
 >
 {totals.sells}
 </span>
 </KpiCell>
...
function KpiCell({ label, children }: { label: string; children: React.ReactNode }) {
 ...
 <div className="mt-0.5 text-[18px] md:text-[20px] tracking-tight text-[var(--ink-strong)]">
 {children}
 </div>
 </div>
 );
...
function WatchlistSectorCard({ group }: { group: SectorGroup }) {
 ...
 valueClass={hasBuySignals ? "text-[var(--gain-700)]" : "text-white/60"}
 />
 <BannerStat
 label="Sell signals"
 value={String(group.sellSignals)}
 valueClass={hasSellSignals ? "text-[var(--loss-700)]" : "text-white/60"}
 />
 </div>
 </div>
 </div>
