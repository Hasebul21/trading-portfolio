...
export function SectorAllocationDetailed({
  ...
  if (visibleSlices.length === 0 || total <= 0) {
    return (
      <div className="rounded-2xl border border-dashed border-zinc-300 bg-white px-6 py-10 text-center text-[14px] text-zinc-500">
    ...
    <div className="flex w-full min-w-0 flex-col gap-6 text-zinc-900">
      ...
      <header className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end">
        ...
        <dl className="grid w-full grid-cols-[auto,1fr] gap-x-3 gap-y-0.5 text-right text-[13px] tabular-nums sm:w-auto sm:gap-x-4">
          <dt className="text-left text-zinc-500 sm:text-right">Total invested</dt>
          <dd className="text-zinc-900">{formatBdt(total)} BDT</dd>
          <dt className="text-left text-zinc-500 sm:text-right">Total drift</dt>
      ...
      <div className="my-6 flex items-center justify-center">
        <div className="relative h-56 w-56" aria-label="Sector allocation donut" role="img">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-center">
            <div className="text-[10px] uppercase leading-snug tracking-[0.16em] text-zinc-500">
              Actual
              <br />
              vs
              <br />
          ...
          <li key={slice.sector} className="flex items-center justify-between gap-3">
            <span className="flex min-w-0 items-center gap-2">
              <span
                ...
                aria-hidden
              />
              <span className="truncate text-zinc-800">{slice.sector}</span>
            </span>
...
function AdjustmentsCard({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
      <header className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 border-b border-zinc-200 px-5 py-4">
        <h2 className="text-[12px] uppercase tracking-[0.14em] text-zinc-500">
      ...
      {!hasAnyTargets ? (
        <p className="px-5 py-8 text-center text-[13px] text-zinc-500">
          Set sector targets in <span className="text-zinc-700">Settings → Sector targets</span> to see rebalance suggestions.
        </p>
      ) : (
        <ul>
          {slices.map((slice, i) => (
  ...
  const actionClass =
    action === "add"
      ...
      : action === "trim"
        ...
        : "text-zinc-500";
  const driftClass =
    driftPct > 0 ? "text-amber-800" : driftPct < 0 ? "text-emerald-700" : "text-zinc-500";
      ...
      <div className="grid grid-cols-2 items-start gap-x-4 gap-y-3 md:grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,1fr))_minmax(0,1.1fr)] md:items-center md:gap-x-6">
        ...
        <div className="col-span-2 flex items-start gap-2 md:col-span-1">
          ...
          <div className="flex flex-col">
            <span className="text-[14px] text-zinc-900">{slice.sector}</span>
        ...
        <StatCell
          ...
          valueClass={slice.targetPercent !== null ? driftClass : "text-zinc-500"}
        />
...
function StatCell({
  ...
  return (
    <div>
      ...
      <div className={`mt-0.5 text-[14px] tabular-nums ${valueClass ?? "text-zinc-900"}`}>
        {value}
      </div>
    </div>
  );
